...
// material
import { Container, Stack, Typography, Card, CardMedia, Box, MenuItem, TextField, Button, Grid, Paper, Radio, RadioGroup, FormControlLabel, FormControl, FormLabel, Snackbar, Checkbox, Tooltip } from '@mui/material';
// components
...
export default function CreateProduct() {
  ...
  const [hasProductCreatePermission, setHasProductCreatePermission] = useState(false);
  const [hasProductEditPermission, setHasProductEditPermission] = useState(false);
  const [hasProductUpdatePermission, setHasProductUpdatePermission] = useState(false);
  ...
  const [hasCategoryListPermission, setHasCategoryListPermission] = useState(false);
  ...
  const setPermissions = (permissions) => {
    ...
    for (let i = 0; i < permissions.length; i++) {
      if (permissions[i].name == "category-list") {
        setHasCategoryListPermission(true)                
    }
      if (permissions[i].name == "brand-list") {
        setHasBrandListPermission(true)                
    }
      if (permissions[i].name === "product-create") {
          setHasProductCreatePermission(true)
      }
            ...
            <Card>
              ...
              >
                  ...
                  <FormControl>
                    ...
                    >
                      ...
                      <Tooltip title={hasBrandListPermission ? "" : "You Do Not Have Permissio to select Brands" }>
                        <FormControlLabel value="yes" control={<Radio />} label="It has a Brand" disabled={hasBrandListPermission ? 0 : 1}/>
                      </Tooltip>
                    </RadioGroup>
                  </FormControl>
                ...
                <Box display="flex"
                  ...
                  <Tooltip title={hasCategoryListPermission ? "" : "You Don't have Permission To select Category"}>
                  <TextField
                    id="outlined-select-currency"
                    select
                    label="Select"
                    ...
                    disabled = {hasCategoryListPermission ? 0 : 1}
                    sx={{ m: 2 }}
                  >
